			...
			struct device_attribute *attr, char *buf)
{
        ...
        mutex_lock(&st->lock);
        res = inv_plat_read(st, REG_TEMP_DATA0_UI, 2, (u8 *)&data);
        mutex_unlock(&st->lock);
        if (res)
                return res;

        temp = (s16)be16_to_cpu(data) * TEMP_SCALE;
        temp = (temp * TEMP_SCALE + TEMP_OFFSET * TEMP_SENSITIVITY) / TEMP_SENSITIVITY;
